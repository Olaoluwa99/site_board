...
class ProjectBloc extends Bloc<ProjectEvent, ProjectState> {
  ...
  ProjectBloc({required UploadProject uploadProject, required GetAllProjects getAllProjects})
  ...
  void _onGetAllProjects(ProjectGetAllProjects event, Emitter<ProjectState> emit) async {
    final response = await _getAllProjects(GetAllProjectsParams(userId: event.userId));
    response.fold(
      (l) => emit(ProjectFailure(l.message)),
